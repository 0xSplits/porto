...
CI=true pnpm vitest src/viem/Key.test.ts

# Run a single test by name
CI=true pnpm vitest -t "behavior: creates key" src/viem/Key.test.ts
```
...
Do NOT manually edit these — they are generated by scripts:

- **`package.json` exports** — generated by `pnpm gen:exports` (via `scripts/gen:exports.ts`). Run after adding/removing/renaming public modules in `src/`.
- **`src/core/internal/_generated/chains.ts`** — generated by `pnpm gen:chains` (via `scripts/chains.ts`). Fetches supported chains from the Relay RPC.
- **`apps/~internal/_generated/contracts.ts`** — generated by `pnpm wagmi generate` from Foundry ABIs.

After installing or changing dependencies, run `pnpm preconstruct` to regenerate exports and build type stubs.
...
Public modules use namespace re-exports in `index.ts` files:
...
Consumer usage: `import { Account, Key } from 'porto/viem'`

Each file is a module. To add a new public module, create the file in the appropriate directory (not under `internal/`) and re-export it from the directory's `index.ts`. Then run `pnpm gen:exports` to update `package.json`.
...
- `VITE_DEFAULT_ENV` controls the RPC environment: `anvil` (default in tests — local blockchain), `stg` (staging), `prod` (production).
- Tests in CI run against both `anvil` and `prod` environments in a matrix.
- Browser tests (`*.browser.test.ts`) spawn Dialog and Playground dev servers automatically via `test/globalSetup.browser.ts`.
...
- `noFloatingPromises: error` — all Promises must be awaited or explicitly voided
- `useSortedClasses: error` — Tailwind classes in `clsx`/`cx`/`cva` must be sorted (run `pnpm check` to auto-fix)
- `useExhaustiveDependencies: error` — React hook dependency arrays must be complete
- Formatter: single quotes, no semicolons, trailing commas, 2-space indent
